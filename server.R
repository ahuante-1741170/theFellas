...
server <- function(input, output) {
  output$incidentsmap <- renderLeaflet({
    ...
    endpoints <- paste0("incidents?page=1&per_page=100&&proximity=",
    ...
    endpoints_locations <- paste0("locations?proximity=", input$location,
    ...
    palette <- colorFactor(palette = c("#ff0000", "#0066ff", "#9966ff"),
      sort(bikes$incidents.type))
  ...
  # Generates plot for correlation between time of day and number of bikes
  # stolen
  data <- reactive({
  ...
  output$incidents_manufacturer <- renderPlot({
    ...
    endpoints_color <- paste0(
      "search?page=1&per_page=100&location=",
